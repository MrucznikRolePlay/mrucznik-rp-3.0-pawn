...
stock Error(const module[], const function[], const info[], playerid=-1)
{
	...
	//
	new logfolder[64];
	format(logfolder, sizeof(logfolder), LOG_FOLDER"Error/%s.log", module);
	Log(logfolder,string);
	
	#if defined DEBUG_MODE_RESTRICT
		printf("DEBUG: Napotkano error przy trybie DEBUG_MODE_RESTRICT - wylaczam serwer ("LOG_FOLDER"/Error/%s.log)", module);
		SendRconCommand("exit");
	#elseif defined DEBUG_MODE_KICK
		if(playerid != -1) 
		{
			printf("DEBUG: Napotkano error przy trybie DEBUG_MODE_KICK - kickuje gracza ("LOG_FOLDER"/Error/%s.log)", module);
			KickEx(playerid);
		}
	#else
		printf("DEBUG: Napotkano error, wiecej informacji w "LOG_FOLDER"/Error/%s.log", module);
	#endif
	return 1;
}
